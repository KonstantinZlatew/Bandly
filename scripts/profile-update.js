(() => {
    ...
    function fillInputs(user)
    {
        ...
        if (els.iUsername) {
            els.iUsername.value = user.username || "";
        }
        if (els.iEmail) {
            els.iEmail.value = user.email || "";
        }
        if (els.iFullName) {
            els.iFullName.value = user.full_name || "";
        }
        if (els.iCountry) {
            els.iCountry.value = user.country || "";
        }
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (els.changePicBtn) {
            els.changePicBtn.addEventListener("click", changeProfilePicture);
        }
        if (els.logoutBtn) {
