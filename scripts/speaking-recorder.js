...
function displayAnalysisResult(result) {
    ...
    if (result.notes) {
        ...
        if (result.notes.FC) {
            html += `<p><strong>FC (Fluency and Coherence):</strong> ${result.notes.FC}</p>`;
        }
        if (result.notes.LR) {
            html += `<p><strong>LR (Lexical Resource):</strong> ${result.notes.LR}</p>`;
        }
        if (result.notes.GRA) {
            html += `<p><strong>GRA (Grammar):</strong> ${result.notes.GRA}</p>`;
        }
        if (result.notes.PR) {
            html += `<p><strong>PR (Pronunciation):</strong> ${result.notes.PR}</p>`;
        }
        html += '</div>';
    }
...
async function checkStatusOnce(submissionId) {
    try {
        ...
        if (submission.status === 'done') {
            ...
            clearInterval(pollInterval);
            pollInterval = null;
            resultsContent.innerHTML = `<div class="error-message"><p>Analysis failed: ${submission.error_message || 'Unknown error'}</p></div>`;
            const btn = document.getElementById('saveRecording');
            btn.disabled = false;
            btn.textContent = 'Save Recording';
            ...
            resultsContent.innerHTML =
                '<div class="processing-message"><p>Recording saved. Waiting for worker to process...</p><p style="font-size: 12px; color: #666; margin-top: 10px;">Note: Make sure speaking-worker.php is running.</p></div>';
        }
    } catch (error) {
        console.error('Polling error:', error);
        resultsContent.innerHTML = `<div class="error-message"><p>Error checking status: ${error.message}</p><p style="font-size: 12px;">Still polling...</p></div>`;
    }
}
